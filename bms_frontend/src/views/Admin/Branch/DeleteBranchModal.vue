  ...
  <div v-if="show" class="modal-overlay" @click="closeModal">
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-body">
        ...
        <p class="warning">This action cannot be undone. Please download the branch data first.</p>

        <div class="download-section">
          <button @click="downloadCSV" class="btn btn-secondary" :disabled="downloaded">
            {{ downloaded ? 'Data Downloaded' : 'Download Branch Data (CSV)' }}
          </button>
        </div>
...
const downloadCSV = () => {
  const csvContent = generateCSV(props.branch)
  const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' })
  const link = document.createElement('a')
  const url = URL.createObjectURL(blob)
  link.setAttribute('href', url)
  link.setAttribute('download', `branch_${props.branch.code}_data.csv`)
...
const generateCSV = (branch: Branch) => {
  let csv = 'Section,Field,Value\n'

  // Branch Info
  csv += `Branch Info,Name,${branch.name}\n`
  csv += `Branch Info,Code,${branch.code}\n`
  csv += `Branch Info,Address,${branch.address || 'N/A'}\n`
  csv += `Branch Info,Contact Phone,${branch.contact_phone || 'N/A'}\n`
  csv += `Branch Info,Contact Email,${branch.contact_email || 'N/A'}\n`
  csv += `Branch Info,Status,${branch.status}\n`
  csv += `Branch Info,Manager,${branch.manager?.name || 'N/A'}\n`
  csv += `Branch Info,Opening Date,${branch.opening_date || 'N/A'}\n`
  csv += `Branch Info,Performance Level,${branch.performance_level}\n`

  // Users
  if (branch.users && branch.users.length > 0) {
    branch.users.forEach((user, index: number) => {
  ...
  // Projects
  if (branch.projects && branch.projects.length > 0) {
    branch.projects.forEach((project, index: number) => {
