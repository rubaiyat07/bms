<template>
  <div class="branch-form">
    <!-- <div class="page-header">
      <h1>{{ isEditing ? 'Edit Branch' : 'Create New Branch' }}</h1>
      <p>{{ isEditing ? 'Update branch information' : 'Add a new branch to the system' }}</p>
    </div> -->

    <div class="form-container">
      <form @submit.prevent="handleSubmit" class="branch-form-content">
        <div class="form-grid">
          <div class="form-group">
            <label for="name">Branch Name *</label>
            <input
              id="name"
              v-model="form.name"
              type="text"
              required
              placeholder="Enter branch name"
              :class="{ 'error': errors.name }"
            />
            <span v-if="errors.name" class="error-message">{{ errors.name }}</span>
          </div>

          <div class="form-group">
            <label for="code">Branch Code *</label>
            <input
              id="code"
              v-model="form.code"
              type="text"
              required
              placeholder="Enter branch code"
              :class="{ 'error': errors.code }"
            />
            <span v-if="errors.code" class="error-message">{{ errors.code }}</span>
          </div>

          <div class="form-group full-width">
            <label for="address">Address *</label>
            <textarea
              id="address"
              v-model="form.address"
              required
              placeholder="Enter full address"
              rows="3"
              :class="{ 'error': errors.address }"
            ></textarea>
            <span v-if="errors.address" class="error-message">{{ errors.address }}</span>
          </div>

          <div class="form-group">
            <label for="contact_phone">Contact Phone</label>
            <input
              id="contact_phone"
              v-model="form.contact_phone"
              type="tel"
              placeholder="Enter contact phone"
              :class="{ 'error': errors.contact_phone }"
            />
            <span v-if="errors.contact_phone" class="error-message">{{ errors.contact_phone }}</span>
          </div>

          <div class="form-group">
            <label for="contact_email">Contact Email</label>
            <input
              id="contact_email"
              v-model="form.contact_email"
              type="email"
              placeholder="Enter contact email"
              :class="{ 'error': errors.contact_email }"
            />
            <span v-if="errors.contact_email" class="error-message">{{ errors.contact_email }}</span>
          </div>

          <div class="form-group">
            <label for="opening_date">Opening Date</label>
            <input
              id="opening_date"
              v-model="form.opening_date"
              type="date"
              :class="{ 'error': errors.opening_date }"
            />
            <span v-if="errors.opening_date" class="error-message">{{ errors.opening_date }}</span>
          </div>

          <div class="form-group">
            <label for="status">Status *</label>
            <select
              id="status"
              v-model="form.status"
              required
              :class="{ 'error': errors.status }"
            >
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
              <option value="suspended">Suspended</option>
            </select>
            <span v-if="errors.status" class="error-message">{{ errors.status }}</span>
          </div>

          <div class="form-group">
            <label for="manager_id">Manager</label>
            <select
              id="manager_id"
              v-model="form.manager_id"
              :class="{ 'error': errors.manager_id }"
            >
              <option value="">Select Manager</option>
              <option v-for="user in managers" :key="user.id" :value="user.id">
                {{ user.name }}
              </option>
            </select>
            <span v-if="errors.manager_id" class="error-message">{{ errors.manager_id }}</span>
          </div>
        </div>

        <div class="form-actions">
          <button type="button" @click="$router.go(-1)" class="btn btn-secondary">Cancel</button>
          <button type="submit" :disabled="loading" class="btn btn-primary">
            <span v-if="loading">Saving...</span>
            <span v-else>{{ isEditing ? 'Update Branch' : 'Create Branch' }}</span>
          </button>
        </div>
      </form>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { api } from '@/services/api'

const route = useRoute()
const router = useRouter()

const isEditing = ref(false)
const loading = ref(false)
const managers = ref([])
const errors = ref({})

const form = ref({
  name: '',
  code: '',
  address: '',
  contact_phone: '',
  contact_email: '',
  opening_date: '',
  status: 'active',
  manager_id: ''
})

const fetchManagers = async () => {
  try {
    const response = await api.get('/users?role=manager')
    if (response.success) {
      managers.value = response.data.data
    }
  } catch (error) {
    console.error('Error fetching managers:', error)
  }
}

const fetchBranch = async (id) => {
  try {
    loading.value = true
    const response = await api.get(`/branches/${id}`)
    if (response.success) {
      const branch = response.data
      form.value = {
        name: branch.name || '',
        code: branch.code || '',
        address: branch.address || '',
        contact_phone: branch.contact_phone || '',
        contact_email: branch.contact_email || '',
        opening_date: branch.opening_date ? branch.opening_date.split('T')[0] : '',
        status: branch.status || 'active',
        manager_id: branch.manager_id || ''
      }
    }
  } catch (error) {
    console.error('Error fetching branch:', error)
  } finally {
    loading.value = false
  }
}

const handleSubmit = async () => {
  try {
    loading.value = true
    errors.value = {}

    const payload = { ...form.value }
    if (!payload.opening_date) delete payload.opening_date
    if (!payload.manager_id) delete payload.manager_id

    let response
    if (isEditing.value) {
      response = await api.put(`/branches/${route.params.id}`, payload)
    } else {
      response = await api.post('/branches', payload)
    }

    if (response.success) {
      router.push('/admin/branches')
    } else {
      errors.value = response.errors || { general: 'An error occurred' }
    }
  } catch (error) {
    console.error('Error saving branch:', error)
    errors.value = { general: 'Failed to save branch' }
  } finally {
    loading.value = false
  }
}

onMounted(() => {
  fetchManagers()

  if (route.params.id) {
    isEditing.value = true
    fetchBranch(route.params.id)
  }
})
</script>

<style scoped>
.branch-form {
  padding: 1rem;
  max-width: 800px;
  margin: 0 auto;
}

.page-header {
  margin-bottom: 2rem;
}

.page-header h1 {
  color: #384e74;
  font-size: 2.5rem;
  font-weight: 700;
  margin-bottom: 0.5rem;
}

.page-header p {
  color: #9CA3AF;
  font-size: 1.125rem;
}

.form-container {
  background: white;
  border-radius: 1rem;
  padding: 2rem;
  border: 1px solid rgba(56, 78, 116, 0.1);
}

.form-grid {
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: 1.5rem;
  margin-bottom: 2rem;
}

.form-group {
  display: flex;
  flex-direction: column;
}

.form-group.full-width {
  grid-column: 1 / -1;
}

.form-group label {
  font-weight: 600;
  color: #384e74;
  margin-bottom: 0.5rem;
}

.form-group input,
.form-group select,
.form-group textarea {
  padding: 0.75rem;
  border: 2px solid rgba(56, 78, 116, 0.2);
  border-radius: 0.5rem;
  font-size: 0.875rem;
  transition: border-color 0.3s ease;
}

.form-group input:focus,
.form-group select:focus,
.form-group textarea:focus {
  outline: none;
  border-color: #384e74;
}

.form-group input.error,
.form-group select.error,
.form-group textarea.error {
  border-color: #EF4444;
}

.error-message {
  color: #EF4444;
  font-size: 0.75rem;
  margin-top: 0.25rem;
}

.form-actions {
  display: flex;
  gap: 1rem;
  justify-content: flex-end;
}

.btn {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.75rem 1.5rem;
  border: none;
  border-radius: 0.5rem;
  font-size: 0.875rem;
  font-weight: 600;
  cursor: pointer;
  transition: all 0.3s ease;
  text-decoration: none;
}

.btn-primary {
  background: linear-gradient(135deg, #8FB3FF 0%, #384e74 100%);
  color: #0F1419;
}

.btn-primary:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 5px 15px rgba(56, 78, 116, 0.3);
}

.btn-primary:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}

.btn-secondary {
  background: transparent;
  border: 2px solid rgba(56, 78, 116, 0.3);
  color: #384e74;
}

.btn-secondary:hover {
  background: rgba(56, 78, 116, 0.1);
  border-color: rgba(56, 78, 116, 0.5);
}

@media (max-width: 768px) {
  .form-grid {
    grid-template-columns: 1fr;
  }

  .form-actions {
    flex-direction: column;
  }
}
</style>
